...
func (g *GenericMulticastProtocolState) MakeAllNonMember() {
	...
	for groupAddress, info := range g.mu.memberships {
		g.makeNonMember(groupAddress, &info)
...
func (g *GenericMulticastProtocolState) InitializeGroups() {
	...
	for groupAddress, info := range g.mu.memberships {
		g.initializeNewMember(groupAddress, &info)
		g.mu.memberships[groupAddress] = info
	}
}
...
func (g *GenericMulticastProtocolState) initializeNewMember(groupAddress tcpip.Address, info *multicastGroupState) {
...
// makeNonMember transitions the given multicast group the the
// non-member/listener state.
func (g *GenericMulticastProtocolState) makeNonMember(groupAddress tcpip.Address, info *multicastGroupState) {
	if info.state == nonMember {
		return
	}
