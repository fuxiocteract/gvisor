...
func (e *endpoint) handlePacket(pkt *stack.PacketBuffer) {
	...
	if !h.IsValid(pkt.Data.Size() + pkt.NetworkHeader().View().Size() + pkt.TransportHeader().View().Size()) {
		...
		statsEP.MalformedPacketsReceived.Increment()
		return
	}
	...
	if header.IsV6MulticastAddress(srcAddr) {
		...
		statsEP.InvalidSourceAddressesReceived.Increment()
		return
	}
	...
	if addressEndpoint := e.AcquireAssignedAddress(dstAddr, e.nic.Promiscuous(), stack.CanBePrimaryEndpoint); addressEndpoint != nil {
		...
		if !e.protocol.Forwarding() {
			...
			statsEP.InvalidDestinationAddressesReceived.Increment()
			return
		}
	...
	if ok := e.protocol.stack.IPTables().Check(stack.Input, pkt, nil, nil, "", ""); !ok {
		...
		statsEP.IPTablesInputDropped.Increment()
		return
	}
	...
	for {
			...
			if extHdr.FragmentOffset() == 0 {
				...
				for {
					...
					if err != nil {
						...
						statsEP.MalformedFragmentsReceived.Increment()
						return
					}
					if done {
						break
		...
		case header.IPv6RawPayloadHeader:
			...
			statsEP.PacketsDelivered.Increment()
...
type Stats struct {
	...
	ICMPSent tcpip.ICMPv6SentPacketStats

	// ICMPRecv holds ICMP statistics of an endpoint.
	ICMPRecv tcpip.ICMPv6ReceivedPacketStats
}
